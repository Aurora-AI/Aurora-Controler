...
## Bloco SVC — Serviços (fixture v3, novo)

3 das 10 lojas ganharam uma **linha de serviços** (`categoria = servico`: conserto/ajuste). Descoberta cega — o motor acha por varredura, não por instrução.

- **SVC1 · Separação.** `servico` é linha distinta: **sem estoque → fora do GMROI**; não pode poluir o ticket médio de produto nem inflar o RFM de produto.
- **SVC2 · Decomposição (crítico).** O resultado de cada loja é decomposto em **produto × serviço (× follow-on)**. Sinalizar toda loja cujo **total é positivo mas cuja margem de PRODUTO é negativa** — serviço mascarando operação de produto quebrada (mesma disciplina do macro D1). Asserção: existe ao menos 1 loja nessa condição e o motor a marca, não a declara saudável.
- **SVC3 · Efeito follow-on / CAC.** Detectar clientes cujo primeiro vínculo foi serviço e que depois compraram produto; o lift existe nas 3 lojas de serviço e é maior em **valor absoluto** na de maior tráfego.
- **SVC4 · Cadastro.** Clientes de serviço têm cadastro completo (captura via conserto).
- **SVC5 · Reconciliação cross-sheet.** `Financeiro.receita_servicos` deve bater com as vendas de `servico` em Vendas; sinalizar meses-loja com gap (há alguns de propósito).
- **Controle:** as 7 lojas sem serviço são grupo de controle; uma loja no prejuízo de produto ficou **sem** serviço de propósito, para comparação direta com a que recebeu.
